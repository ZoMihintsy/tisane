...
<div class="dashboard-section">
  ...
  @if($client->isEmpty())
    <p class="text-center">
      Il n'y a pas d'utilisateur pour l'instant
    </p>
  @else
  <div class="card-grid">
    @foreach($client as $clients)
      <div class="card category-card">
        <svg width="60" height="60" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
            <circle cx="100" cy="100" r="90" stroke="black" stroke-width="8" fill="none"/>
            <circle cx="100" cy="70" r="30" fill="black"/>
            <path d="M70 140 C70 110, 130 110, 130 140 L130 170 C130 180, 70 180, 70 170 Z" fill="black"/>
          </svg>
        <h3 style="text-transform: capitalize">{{$clients->name}}</h3>
        <a href="mailto:{{$clients->email}}">{{$clients->email}}</a> 
        <div class="card-actions">
        @if($clients->type == "client")
        <a href="{{route('client.dtach',['id'=>$clients->id])}}" ><button>Detacher le client</button></a>
        @else
        <a href="{{route('client.atach',['id'=>$clients->id])}}"><button>Attacher le client</button></a>
        @endif
        </div>
      </div>
    @endforeach
