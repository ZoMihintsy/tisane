<div>
    ...
    <form action="{{route('guest.panier',['id'=>$produits->id])}}" method="post">
        @csrf
        @method('put')
        <x-text-input type="text" class="w-full" name="nom" placeholder="{{ __('renseignement.nom_commande') }}" />
        <x-input-error :messages="$errors->get('nom')" />
        <br>
        <br>
        <x-text-input type="email" class="w-full" name="email" placeholder="{{ __('renseignement.email_commande') }}" />
        ...
        <br>
        <br>
        <x-text-input type="number" class="w-full" name="stock" placeholder="Nombre à prendre" />
